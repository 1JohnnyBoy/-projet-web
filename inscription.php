...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    elseif(empty($_POST['email']) || empty($_POST['password'])){ /*askip méthode plus fiable car elle ne dépendra pas du bouton(name="valider")*/
        $message = "Veuillez remplir tous les champs";
    }
    else{
        
        $nom = trim((string) ($_POST['nom'] ?? ''));/*récup le nom saisi dans le formulaire*/
        $email = trim((string) ($_POST['email'] ?? ''));/*récup l'email saisi dans le formulaire*/ 
        $password_hache = password_hash($_POST['password'], PASSWORD_DEFAULT); /*pour crypter le mdp*/
